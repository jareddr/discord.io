	...
	self.getAudioContext = function(channelObj, callback) { //Thanks #q/qeled.
		checkRS(function() {
			...
			function AudioCB(audioChannels, opusEncoder, udpClient, vWS) {
				...
				this.playAudioFile = function(location, volume, seek) {
					var volume = volume || 1
					var childProc, spawn, spawnSync, encs = ['ffmpeg', 'avconv'], selection, enc;
					var existingStream = false;
					if (playingAF) {
						existingStream = streamRef
					}
					...
					if (selection) {
						...
						streamRef = enc;
						if(existingStream){
							console.log("Killing existing stream.");
							existingStream.stdout.pause();
							existingStream.stdout.end();
				...
				this.stopAudioFile = function(callback) {
					if (playingAF) {
						...
						streamRef.stdout.pause();
						streamRef.stdout.end();
						streamRef.kill('SIGKILL');
						streamRef.kill();
						playingAF = false;
					} else {
